...
def get_timeseries_data_from_location(
    covId: str, tsvBaseURL: str, start: datetime, end: datetime, lat: float, lon: float
):
...
def download_variable_parcela(
    ...
    raw_data = get_timeseries_data_from_location(
        covId=variable,
...
def main():
    ...
    for _, row in df_parcelas.iterrows():
        ...
        start_time = datetime.strptime(
            row["fecha_primera_muestra"].strftime("%d-%m-%Y"), "%d-%m-%Y"
        )
        end_time = datetime.strptime(
            row["fecha_ultima_muestra"].strftime("%d-%m-%Y"), "%d-%m-%Y"
        )
        ...
        for variable in variables_list:
            df_variable = download_variable_parcela(
                variable, start_time, end_time, lat_parcela, lon_parcela
            )

            filename = f"{id_parcela}_{variable}_{start_time_document}_{end_time_document}.txt".replace(  # noqa: E501
                "/", ""
            )
