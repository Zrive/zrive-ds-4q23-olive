from processing import build_spine, build_numeric_features_parcela, build_binary_features_parcela, build_date_variables_parcelas, build_categorical_features_parcela, build_binary_features_parcela, estados
import numpy as np
import pandas as pd
...
def train_test_val_split(df:pd.DataFrame, train_size=0.7, test_size = 0.2, cooloff_days = 14):
    ...
    val_df = df.loc[df['fecha'].between(df_val_index + pd.DateOffset(days=cooloff_days),df_test_index, inclusive="left")]
